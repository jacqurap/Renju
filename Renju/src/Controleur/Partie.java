...
public class Partie implements Serializable {
    ...
    public boolean action(Point p) { //TODO
        ...
        int[][] newGrille = newPlateau.getGrille();
        int[][] oldGrille = plateau.getGrille();
        for (int i = 0; i < 15; i++) {
            for (int j = 0; j < 15; j++) {
        ...
        if (nbCoups % 2 == 0) {
            newPlateau.setCase(p.x, p.y, plateau.CASENOIRE);
            if (ia2) {
                touria = true;
            }
        } else {
            newPlateau.setCase(p.x, p.y, plateau.CASEBLANCHE);
            if (ia1) {
                touria = true;
            }
        }
    ...
    public void setPlateau() {
        this.plateau = this.annuler.peek();
    }
}
